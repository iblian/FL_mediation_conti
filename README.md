...
Input:
The input data should contain the (correlation (X,M), correlation (X,Y), correlation (M,Y), mean_X, mean_M, mean_Y, sd_X, sd_M, sd_Y, sample_size_n) from each source. The example data shows the format of data to be input.
Output: 
...
2. he direct effect is c-ab, and propotion of mediation effect (ab/c).
